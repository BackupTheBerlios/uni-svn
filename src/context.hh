...
namespace NAMESPACE
{
  ...
  public:
    ...
    /**
     * Evaluate code in an input stream.
     * This function uses the scanner to scan the input stream,
     * and internally call reduce_in_shield to reduce terms scanned.
     * Therefore, all the exceptions will be handled by handlers
     * defined inside the input code, and only be thrown outside
     * when there are not handlers for them.
     *
     * \param   scanner   the pointer to the scanner which is to be used.
     *
     ...
     *          otherwise the original term is returned.
     */
    TermPtr eval (Scanner* scanner, std::istream& input, int flags = ALL);
     ...
     *
     * \param   lib   the name of the library; this parameter will be
     *                passed to the handler directly, with no interpretion.
     */
    void import (const string& lib);
